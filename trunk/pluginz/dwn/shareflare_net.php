...
if (($_GET ["premium_acc"] == "on"  && $_GET ["premium_pass"]) || ($_GET ["premium_acc"] == "on" && $premium_acc ["shareflare"] ["pass"])) {
 ...
 $page = geturl($Url["host"], $Url["port"] ? $Url["port"] : 80, $Url["path"].($Url["query"] ? "?".$Url["query"] : ""), $LINK, 0, 0, 0, $_GET["proxy"],$pauth);
    ...
    $PreForm = cut_str ( $page ,'action="/sms/check2.php' ,'</form>' );
    ...
    $UrlAct="http://shareflare.net/sms/check2.php";
    $post['pass']=$_GET ["premium_pass"] ? $_GET ["premium_pass"] : $premium_acc ["shareflare"] ["pass"];
    $post['uid5']=$uid5;
    $post['uid']=$uid;
    $post['name']=$name;
    $post['pin']=$pin;
...
if (preg_match('%href=.*?(http://.+?downloadp.+?)[\'"]%i', $page, $regs)) {
	$dwnl = $regs[1];
} else {
...
if ($_POST['step'] == 1) {
    ...
    $UrlAct="http://shareflare.net/download3.php";
    $post = unserialize(urldecode($_POST["post"]));
	$post['uid2']=$post['uid'];
    $post['cap']=$_POST["captcha"];
    $cookie = urldecode($_POST['cookie']);
    $Url = parse_url($UrlAct);
    $Referer = "http://shareflare.net/download4.php";
    ...
    */
    ...
    $Url=parse_url("http://shareflare.net/tmpl/tmpl_frame_top.php?link=");
  $page = geturl($Url["host"], $Url["port"] ? $Url["port"] : 80, $Url["path"].($Url["query"] ? "?".$Url["query"] : ""),"http://shareflare.net/tmpl/tmpl_frame_top.php?link=", $cookie, 0, 0, $_GET["proxy"],$pauth);
is_page($page);
$wait=trim(cut_str ( $page ,'y = ' ,';' ));
insert_timer($wait,"","Please wait");
$Url=parse_url("http://shareflare.net/tmpl/tmpl_frame_top.php");
$page = geturl($Url["host"], $Url["port"] ? $Url["port"] : 80, $Url["path"].($Url["query"] ? "?".$Url["query"] : ""),"http://shareflare.net/tmpl/tmpl_frame_top.php?link=", $cookie, 0, 0, $_GET["proxy"],$pauth);
is_page($page);

if (preg_match('/DownloadClick.+href="(.*)"/i', $page, $regs)) {
	$dwn = $regs[1];
} else {
	html_error("download link not found . Please reattent another time");
}
...
insert_location("index.php?filename=".urlencode($FileName)."&force_name=".urlencode($FileName)."&host=".$Url["host"]."&port=".$Url["port"]."&path=".urlencode($Url["path"].($Url["query"] ? "?".$Url["query"] : ""))."&referer=".urlencode($Referer)."&cookie=".urlencode($cookie)."&email=".($_GET["domail"] ? $_GET["email"] : "")."&partSize=".($_GET["split"] ? $_GET["partSize"] : "")."&method=".$_GET["method"]."&proxy=".($_GET["useproxy"] ? $_GET["proxy"] : "")."&saveto=".$_GET["path"]."&link=".urlencode($LINK).($_GET["add_comment"] == "on" ? "&comment=".urlencode($_GET["comment"]) : "").($pauth ? "&pauth=$pauth" : "").(isset($_GET["audl"]) ? "&audl=doum" : ""));
    ...
    $uid = cutOn($FreeForm,'name="uid" value="','"',2);
...
if (stristr($page,"cap.php?"))
    {
    $imagecode = cut_str($page,"<img src='http://shareflare.net/cap.php?jpg=","'");
    $img = 'http://shareflare.net/cap.php?jpg='.$imagecode;
    $Url = parse_url($img);
    ...
    if($options["download_dir"]){
          $imgfile=$options["download_dir"]."shareflare_captcha.jpg";    
        }else{
              $imgfile=download_dir."shareflare_captcha.jpg";
        }
    ...
    $code .= 'Please enter : <img src="'.$download_dir.'shareflare_captcha.jpg?'.rand(1,10000).'"><br><br>'.$nn;
...
function cutOn($str, $left, $right,$cont=1)
    {
...
/*************************\
 WRITTEN BY KAOX 23-feb-10
 Fix by kaox 19-may-10
\*************************/
?>
