...
if (($_GET["premium_acc"] == "on" && $_GET["premium_user"] && $_GET["premium_pass"]) || ($_GET["cookieuse"] == "on" && $_GET["cookie"]) || ($_GET["premium_acc"] == "on" && $premium_acc["ugotfile"]["user"] && $premium_acc["ugotfile"]["pass"])) {
    if ($_GET["cookieuse"] == "on" && $_GET["cookie"]) {
        $cookie = $_GET["cookie"];
    } else {
        $Url = parse_url("http://ugotfile.com/user/login");
        $post["ugfLoginUserName"] = $_GET ["premium_user"] ? $_GET ["premium_user"] : $premium_acc ["ugotfile"] ["user"];
        $post["ugfLoginPassword"] = $_GET ["premium_pass"] ? $_GET ["premium_pass"] : $premium_acc ["ugotfile"] ["pass"];
