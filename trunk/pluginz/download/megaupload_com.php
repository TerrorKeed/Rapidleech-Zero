...
class megaupload_com extends DownloadClass {
    ...
    public function Download($link) {
        ...
        if (preg_match("/f=(\w+)/", $Url ["query"], $matches)) {
            ...
            $this->moveToAutoDownloader($matches[1]);
        }
        //Redirect
    ...
    private function DownloadPremium($link) {
        ...
        $post = array();
        $post ['login'] = 1;
        $post ["username"] = $_REQUEST["premium_user"] ? trim($_REQUEST["premium_user"]) : $premium_acc ["megaupload"] ["user"];
        $post ["password"] = $_REQUEST["premium_pass"] ? trim($_REQUEST["premium_pass"]) : $premium_acc ["megaupload"] ["pass"];
