...
if (($_GET["premium_acc"] == "on" && $_GET["premium_user"] && $_GET["premium_pass"]) || ($_GET["premium_acc"] == "on" && $premium_acc["freakshare"]["user"] && $premium_acc["freakshare"]["pass"])) {
    $Url = parse_url("http://freakshare.com/login.html");
    $post["user"] = ($_GET["premium_user"] ? $_GET["premium_user"] : $premium_acc["freakshare"]["user"]);
    $post["pass"] = ($_GET["premium_pass"] ? $_GET["premium_pass"] : $premium_acc["freakshare"]["pass"]);
    $post["submit"] = "Login";
    ...
    if ($_GET ["step"] != "second") {
        if ($_GET ["step"] != "cu") {
            ...
            if (!$info) {
                $info = cut_str($page, '<h1 class="box_heading" style="text-align:center;">', '</h1>');
            }
...
//$k = "6Le1iwoAAAAAANwh1QwMQkA0eGvqbHkaKZO8FxFy";
            ...
            $arcapt = apireca($k, $download_dir, $cookie, "freakshare");
            $post['recaptcha_challenge_field'] = $arcapt[rcf];

            $code = '<center>';
            $code .= '<form id="regularForm" method="post" action="' . $PHP_SELF . (isset($_GET["idx"]) ? "&idx=" . $_GET["idx"] : "") . '">' . $nn;
...
function apireca($k, $optionsd, $cookies, $sr) {
    ...
    if (!$rcf) {
        html_error("Error getting captcha", 0);
    }
    $cookies .= "; " . $cookie;
    $arcapt = array();
