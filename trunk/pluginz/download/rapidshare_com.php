...
class rapidshare_com extends DownloadClass {
	...
	public function Download($link) {
		...
		if (($_REQUEST['cookieuse'] == 'on' && preg_match('@enc\s?=\s?(\w+)@i', $_REQUEST['cookie'], $c)) || ($_REQUEST['premium_acc'] == 'on' && !empty($premium_acc['rapidshare_com']['cookie'])) || ($_REQUEST["rs_acc"] == "on" && (!empty($_GET["rs_cookie"]) || !empty($_GET["rs_hash"])))) {
			if (!empty($_GET["rs_cookie"])) $this->cookie = $_GET["rs_cookie"];
			elseif (!empty($_GET["rs_hash"])) $this->cookie = strrev(dcd($_GET["rs_hash"]));
			else $this->cookie = (empty($c[1]) ? $premium_acc['rapidshare_com']['cookie'] : $c[1]);
			$this->CheckLogin();
