...
if ($storage_limit > 0) {
    if ($serverfiles > ($storage_limit * 1024 * 1024)) {
        $storage_exceed = true;
    }
}
...
if ($is_expired || $is_exceed || !$is_worktime || $storage_exceed || $alert_sloadhigh || $is_dllimit) {
    ...
    if ($is_dllimit) {
        $msg.= ( $msg != "" ? "<br>" : "You have exceeded your download limit, you can only download " . $downloadsPerIP . " files in " . sec1time($downloadDelayPerIP) . "." );
