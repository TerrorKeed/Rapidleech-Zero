...
public function Download($link) {
        global $premium_acc, $rs_cookie_enc_value;
        if ($_REQUEST["rs_acc"] == "on" && (!empty($_GET["rs_cookie"]) || !empty($_GET["rs_hash"]) || !empty($rs_cookie_enc_value))) {
                ...
                return $this->DownloadPremium($link);
        } else {
...
private function PremiumCookieDownload($fileid, $filename, $cookie) {
        ...
        if (stristr($page, "Location:")) {
                ...
                html_error("Cannot use premium cookie");
        }
}
private function Check_Limit($page, $ret = false) {
        ...
        if (preg_match("/X-APICPU: (\d+)\/(\d+)/i", $header, $x)) {
                ...
                if (($api_remaining - $api_used) <= $api_min) {
                        echo('<br />Warning: Too much Rapidshare APICPU usage.<br />');
...
//[20-MAR-11] Plugin edited to work in 36B (With cookies & hashes) & Added functions (Edited) from DownloadClass. - Th3-822
