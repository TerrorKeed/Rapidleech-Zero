...
if (($_GET["premium_acc"] == "on" && $_GET["pr_user"] && $_GET["pr_pass"]) || ($_GET["premium_acc"] == "on" && $premium_acc["megashares"]["user"] && $premium_acc["megashares"]["pass"]))
{
	$post = Array();
	$post["mymslogin_name"] = (($_GET["pr_user"] && $_GET["pr_pass"]) ? $_GET["pr_user"] : $premium_acc["megashares"]["user"]);
	$post["mymspassword"] = (($_GET["pr_user"] && $_GET["pr_pass"]) ? $_GET["pr_pass"] : $premium_acc["megashares"]["pass"]);
	...
	is_present($page, "Password does not match Username", "Error during login - Password does not match Username.");
...
// update by guntur_roxx 26 May 2010 (with idoenk helping me of course :hammer:)
?>
