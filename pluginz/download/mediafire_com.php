...
$page = "";
foreach ($pages as $v)
    $page .= mf_str_conv($v . "eval");
if (!preg_match("/=\W?(\w+)[\('\"]+(\w{11,})[,'\"]+([0-9a-f]{33,})/i", $page, $eb))
    html_error('Error 1.1');
if (!$pageb = cut_str($page, $eb[1] . '(', ';}}'))
    html_error('Error 1.2');
if (!preg_match("|getElementById\(.([0-9a-f]{32}).|", $pageb, $match))
    html_error('Error 1.3');
if (!$pKr = cut_str($page, "pKr='", "'"))
    html_error('Error 1.4');
...
foreach ($pages as $v)
    $page .= mf_str_conv($v . "eval");
if (!$tmp = cut_str($page, $match[1], '</xmp>'))
    html_error('Error 2.0');
if (!preg_match("|http:[^\"']+(.\+(\w+)\+.)[^\"']+|", stripslashes(str_replace(" ", "", $tmp)), $link_match))
    html_error('Error get download link');
if (!preg_match("/" . $link_match[2] . "=.([\w]+)/", $page, $match)) {
    $pagea = stripslashes(str_replace(" ", "", $pages[0]));
    if (!preg_match("/" . $link_match[2] . "=.([\w]+)/", $pagea, $match))
        html_error('Error 2.5');
}
