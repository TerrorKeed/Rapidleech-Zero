...
class filefactory_com extends DownloadClass {
    ...
    public function Download($link) {
        ...
        if (($_REQUEST["premium_acc"] == "on" && $_REQUEST["premium_user"] && $_REQUEST["premium_pass"]) || ($_REQUEST["premium_acc"] == "on" && $premium_acc["filefactory"]["user"] && $premium_acc["filefactory"]["pass"])) {
    ...
    private function Prepare_Free($link) {
        ...
        if (preg_match('/check:\'([^\']+)/i', $page, $ck) && preg_match('/Recaptcha\.create\("([^"]+)/i', $page, $pid)) {
    ...
    private function Download_Free($link) {
        ...
        is_present($page, "<strong>temporarily limited</strong>", "Your access to the free download service has been <strong>temporarily limited</strong> to prevent abuse... Please wait 10 minutes or more and try again.");
    ...
    private function Download_Premium($link) {
        ...
        if ($_REQUEST["premium_user"] && $_REQUEST["premium_pass"]) {
            $email = $_REQUEST["premium_user"];
            $password = $_REQUEST["premium_pass"];
        } else {
            $email = $premium_acc["filefactory"]["user"];
            $password = $premium_acc["filefactory"]["pass"];
        }
        ...
        $page = $this->GetPage($link, 0, 0, 0, base64_encode($email . ":" . $password));
    ...
    private function login($email, $password, $chkprem = true) {
        ...
        is_notpresent($cookie, "ff_membership=", "Login Failed: Cannot get cookie.");
...
//[19-Mar-2011] [20-Mar-2011] Plugin edited to work in 36B & Added functions (Edited) from DownloadClass && Moved "Download($LINK);" to last line & some little edits in captcha function (Removed a extra slash & changed reCAPTCHA theme to 'white' :D ). - Th3-822
?>