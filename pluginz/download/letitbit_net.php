...
class letitbit_net extends DownloadClass {
    ...
    public function Download($link) {
        ...
        unset($page);
        if (($_REQUEST ['premium_acc'] == 'on' && $_REQUEST['premium_user'] && $_REQUEST ['premium_pass']) || ($_REQUEST ['premium_acc'] == 'on' && (!empty($premium_acc ['letitbit'] ['user']) && !empty($premium_acc ['letitbit'] ['pass'])))) {
            $this->Login($link);
        } else {
            ...
            if (($_REQUEST['premium_acc'] == 'on' && $_REQUEST['premium_pass']) || ($_REQUEST['premium_acc'] == 'on' && (!empty($premium_acc['letitbit_net']['pass'])))) {
                $post['pass'] = $_REQUEST["premium_pass"] ? trim($_REQUEST["premium_pass"]) : $premium_acc ["letitbit"] ["pass"];
    ...
    private function Login($link) {
        ...
        $user = ($_REQUEST ["premium_user"] ? $_REQUEST ["premium_user"] : $premium_acc ["letitbit"] ["user"]);
        $password = ($_REQUEST ["premium_pass"] ? $_REQUEST ["premium_pass"] : $premium_acc ["letitbit"] ["pass"]);
