...
class megashare_com extends DownloadClass {
    ...
    public function Download($link) {
        ...
        if (($_REQUEST["premium_acc"] == "on" && $_REQUEST["premium_user"] && $_REQUEST["premium_pass"]) || ($_REQUEST["premium_acc"] == "on" && $premium_acc["megashare"]["user"] && $premium_acc["megashare"]["pass"])) {
    ...
    private function Prepare_Free($link) {
        ...
        if (!preg_match_all('@<input type="hidden" name="([^"]+)" value="(?#Still hating chunked reqs)(?:\r?\n[^\r|\n]+\r?\n)?([^"]+)?"(?:\s?/?)>@i', $page, $hI))
            html_error("Error getting POST data 2.");
    ...
    private function Download_Premium($link) {
        ...
        if (!preg_match('@<input type="hidden" name="([^"]+)" value="([^"]+)">@i', $page, $HV) || !preg_match('@src="images/download-btn\.gif" name="([^"]+)" class="textfield" value="([^"]+)">@i', $page, $SP))
            html_error("[Premium] Error getting POST data.");
        ...
        $form = cut_str($page, 'name="downloader">', "</form>");
        if (!preg_match_all('@<input type="hidden" name="([^"]+)" value="(?#Still hating chunked reqs)(?:\r?\n[^\r|\n]+\r?\n)?([^"]+)?"(?:\s?/?)>@i', $page, $hI))
            html_error("[Premium] Error getting POST data 2.");

        if (!preg_match('@src="images/get-direct-link-btn.png" name="([^"]+)" value="([^"]+)"@i', $page, $dI))
            html_error("[Premium] Error getting POST data 3.");
        $post = array($dI[1] . '.x' => 1, $dI[1] . '.y' => 1);
        $hI = array_combine($hI[1], $hI[2]);
        foreach ($hI as $k => $v) {
            $post[$k] = $v;
