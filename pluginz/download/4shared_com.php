...
class d4shared_com extends DownloadClass {
    ...
    public function Download($link) {
        ...
        if ($_REQUEST["premium_acc"] == "on" && (($_REQUEST["premium_user"] && $_REQUEST["premium_pass"]) || ($premium_acc["4shared"]["user"] && $premium_acc["4shared"]["pass"]))) {
    ...
    private function login($pA=false) {
        global $premium_acc, $htxt;
        $email = ($pA ? $_REQUEST["premium_user"] : $premium_acc["4shared"]["user"]);
        $pass = ($pA ? $_REQUEST["premium_pass"] : $premium_acc["4shared"]["pass"]);
        ...
        $this->changeMesg($htxt['_retrieving'] . "<br />4S Premium Download<br />Bandwidth: $used% of $total {$qm[3]}.");
