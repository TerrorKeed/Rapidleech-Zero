...
class ifolder_ru extends DownloadClass {
    ...
    public function Download($link) {
        ...
        if ($_GET["action"] == "1") {
            ...
            if (!preg_match_all('@http://ints.ifolder.ru/ints/sponsor/\?bi=\d+&session=(\w+)&u=(http%3A%2F%2F[^>|"|\']+)@i', $page, $match))
                html_error("Ads not found.");
            $sesid = $match[1][0];
            ...
            if (!preg_match('@var delay = (\d+);@i', $page, $T))
        ...
        if (!preg_match('@id="download_file_href" href="(http://stg\d+.ifolder.ru/download/[^"]+)"@i', $page, $D))
