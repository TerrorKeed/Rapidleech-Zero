...
class oron_com extends DownloadClass {
    ...
    public function Download($link) {
        global $premium_acc;
        if (($_REQUEST ["premium_acc"] == "on" && $_REQUEST ["premium_user"] && $_REQUEST ["premium_pass"]) || ($_REQUEST ["premium_acc"] == "on" && $premium_acc ["oron"] ["user"] && $premium_acc ["oron"] ["pass"])) {
    ...
    private function Retrieve($link) {
        ...
        $this->EnterCaptcha($download_dir . "oron_captcha.jpg", $data, 10);
        exit();
    }
    ...
    private function DownloadPremium($link) {
        ...
        $post = array();
        $post['login'] = $_REQUEST["premium_user"] ? trim($_REQUEST["premium_user"]) : $premium_acc ["oron"] ["user"];
        $post['password'] = $_REQUEST["premium_pass"] ? trim($_REQUEST["premium_pass"]) : $premium_acc ["oron"] ["pass"];
        $post['op'] = "login";
        $post['redirect'] = $link;
        $post['rand']="";
