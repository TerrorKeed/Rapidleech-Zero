...
class uploaded_net extends DownloadClass {
	...
	private function login($loginc=false) {
		...
		if (!preg_match_all('/class="cB">([\d\,]+)[\s\t]? ([\w]?B)<\/\w+><\/th>/i', $quota, $tr)) html_error('Error[Form account profile may have changed!]');

		$this->changeMesg($L->say['_retrieving'] . "<br />Uploaded.net Premium Download<br />Traffic: For Downloading: {$tr[1][0]} {$tr[2][0]}, Flexible usable contingent for DDL: {$tr[1][1]} {$tr[2][1]}, Total: {$tr[1][2]} {$tr[2][2]}.");
 ...
 * Fixed cookie error also check bandwith limit by Tony Fauzi Wihana/Ruud v.Tony 1-2-2013 with help Th3-822
 */
?>
