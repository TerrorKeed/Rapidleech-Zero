...
class rapidshare_com extends DownloadClass {
	...
	public function Download($link) {
        ...
        if ($_REQUEST["rs_acc"] == "on" && (!empty($_GET["rs_cookie"]) || !empty($_GET["rs_hash"]) || !empty($premium_acc["rapidshare_com"]["cookie"]))) {
            if (!empty($_GET["rs_cookie"])) {
                $cookie = $_GET["rs_cookie"];
            } elseif (!empty($_GET["rs_hash"])) {
                $cookie = strrev(dcd($_GET["rs_hash"]));
            } else {
                $cookie = $premium_acc["rapidshare_com"]["cookie"];
            }
			$this->lastmesg = $L->say['_retrieving']."<br />RS Premium Download [Cookie]";
			...
			$this->lastmesg = $L->say['_retrieving']."<br />RS Premium Download";
			$this->changeMesg($this->lastmesg);

			return $this->DownloadPremium();
		} else {
			$this->lastmesg = $L->say['_retrieving'].'<br />RS Free Download';
			$this->changeMesg($this->lastmesg);
